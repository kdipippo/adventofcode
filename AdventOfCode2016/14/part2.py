...
def keyStretching(salt, saltIndex):
    finalHash = hashlib.md5(salt + str(saltIndex)).hexdigest()
    for i in range(2016):
...
def calcHash(salt):
    ...
    for i in range(1001):
        allHashes.append(keyStretching(salt,i))
    while len(validHashes) < 64:
        ...
        if bool3: # passes containsThree
            ...
            if validThousandHashes(char3,allHashes[saltIndex+1:]):
                ...
                print "CURRENTLY AT " + str(len(validHashes))
        allHashes.append(keyStretching(salt,saltIndex+1001))
        saltIndex += 1
    return validHashes
