...
class SeedsTracker:
    ...
    def migrate_rows_after_processing(self):
        ...
        for entry in self.after_nums:
            if entry not in self.before_nums:
                self.before_nums.append(entry)
        self.after_nums = []

def parse_seeds(seeds_str):
    split_list = [int(entry) for entry in seeds_str.replace("seeds: ", "").split()]
    seeds = []
    for index in range(0, len(split_list), 2):
        seeds.append(split_list[index])
        seeds.append(split_list[index] + split_list[index+1])
    return seeds

def parse_seeds2(seeds_str):
    split_list = [int(entry) for entry in seeds_str.replace("seeds: ", "").split()]
    seeds = array('L', [])
    for index in range(0, len(split_list), 2):
        print(f"  Range #{index} out of {len(split_list)}")
        seeds.extend(array('L', [i for i in range(split_list[index], split_list[index] + split_list[index+1])]))
    return seeds
